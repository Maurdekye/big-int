...
impl<const BASE: usize> Normalize for Tight<BASE> {
...
impl<const BASE: usize> From<TightBuilder<BASE>> for Denormal<Tight<BASE>> {
...
mod tests {
    ...
    fn get_digit() {
        ...
        builder.push_front(1);
        let int: Tight<10> = builder.build();
    ...
    fn set_digit() {
        ...
        builder.push_back(3);
        builder.push_back(4);
        let mut int: Tight<10> = builder.build();
        int.set_digit(1, 8);
        int.set_digit(2, 0);
        assert_eq!(int.get_digit(0), Some(1));
