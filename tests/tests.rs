...
mod tests {
    ...
    fn division_2() {
        ...
        let b = 6331.into();
        assert_eq!(a.div_rem_lowmem(b), Ok(((-3).into(), (-6113).into())));
    }
    ...
    fn division_3() {
        let a: BigInt<10> = (-27792).into();
        let b = 6.into();
        assert_eq!(a.div_rem_lowmem(b), Ok(((-4632).into(), 0.into())));
    }
    ...
    fn division_by_zero() {
        let a: BigInt<10> = 999_999_999.into();
        let b = 0.into();
        assert_eq!(a.div_rem_lowmem(b), Err(BigIntError::DivisionByZero));
    }
    ...
    fn fuzzy_division_test() {
        ...
        for _ in 0..100_000 {
            ...
            if b > 0 {
                assert_eq!(
                    BigInt::from(a).div_rem_lowmem(BigInt::from(b)),
    ...
    fn fuzzy_base_256_division_test() {
        ...
        for _ in 0..100_000 {
            ...
            if b > 0 {
                assert_eq!(
                    BigInt::<256>::from(a).div_rem_lowmem(b.into()),
    ...
    fn fuzzy_div_rem_2_test() {
        ...
        for _ in 0..100_000 {
            ...
            if b > 0 {
                assert_eq!(
                    BigInt::<10>::from(a).div_rem(b.into()),
                    Ok(((a / b).into(), (a % b).into())),
                    "{a} / {b}"
                );
            }
    ...
    fn conversion_test_2() {
        assert_eq!(
            BigInt::<10>::from(-7935368386145574994_isize).convert(),
            BigInt::<16>::from(-7935368386145574994_isize)
        );
    }
    ...
    #[test]
    fn fuzzy_lowmem_conversion_test_10_to_16() {
        let mut rng = thread_rng();
        for _ in 0..10_000 {
            let n = rng.gen::<i64>() as i128;
            assert_eq!(
                BigInt::<10>::from(n).convert_lowmem(),
                BigInt::<16>::from(n),
                "{n}"
            )
        }
